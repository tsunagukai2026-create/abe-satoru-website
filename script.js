  ...
  const card = item => {
    ...
    return `
      <article class="text-card ${item.image ? 'has-image' : ''}">
        ${item.image ? `<div class="text-card-image-wrap"><img src="${item.image}" alt="${item.title}" class="text-card-image ${portrait ? 'portrait' : ''}"></div>` : ''}
        <div class="text-card-body">
          <h3>${item.title}</h3>
          <p>${item.text || ''}</p>
          ${item.url ? `<a href="${item.url}" target="_blank" rel="noopener">VIEW MORE →</a>` : ''}
        </div>
      </article>`;
  };

  document.getElementById('themes-grid').innerHTML = data.themes.map(card).join('');
  document.getElementById('works-grid').innerHTML = data.works.map(card).join('');
  document.getElementById('social-grid').innerHTML = data.social.map(card).join('');

  document.getElementById('news-list').innerHTML = data.news.map(item => `
